 ...
 ** @brief Class for reconstruction of the full event topology including primary
 *vertices and short-lived particles.
 ** @author  I.Kisel, M.Zyzak
 ** @date 05.02.2019
 ** @version 1.0
 **
 ** The class receives as an input tracks from the track finder (both at first
 *and last hit positions)
 ** with the PDG hypothesis, reconstructs primary vertex candidates, divides
 *tracks into secondary and
 ** primary, each of these groups are subdivided into positive and negative
 *tracks, then they are
 ** sorted according to the PDG hypothesis, short-lived particles are
 *constructed, optionally competition
...
public:
...
#ifdef KFPWITHTRACKER
  /** Copies tracks from the standalone CA track finder to the vector
   *KFParticleTopoReconstructor::fTracks
   ** assuming a given pdg hypothesis for each track. Tracks are rotated to the
   *global coordinate system
   ** and checked to have finite parameters. The
   *KFParticleTopoReconstructor::fKFParticlePVReconstructor
   ** is initialised with the copied tracks.
   ** \param[in] tracker - the standalone CA track finder
   ** \param[in] pdg - pointer to the vector with PDG hypothesis for each track,
   ...
   *KFParticleTopoReconstructor::fTracks
   ** assuming a given PDG hypothesis for each particle. If pointer to
   *pdg-vector is not provided
   ** "-1" PDG is assigned to each input particle. If pointer to number of
   ...
   *Only reconstruction
   ** of short-lived particles should be run if this initialisation method is
   *used.
   ** After reconstruction of short-lived particles the
   *KFParticleTopoReconstructor::DeInit() method
   ** should be called, otherwise an external memory will be released that can
   *lead to the memory
  ...
  void AddPV(const KFVertex &pv, const std::vector<int> &tracks) {
     ...
     ** \param[in] tracks - vector with indices of tracks associated with the
     *provided primary vertex.
     **/
...
private:
   ...
   *For reconstruction of short-lived particles
   ** Tracks should be sorted by the KFParticleTopoReconstructor::SortTracks()
   *function. The tracks after sorting are divided
   ** into several groups: \n
