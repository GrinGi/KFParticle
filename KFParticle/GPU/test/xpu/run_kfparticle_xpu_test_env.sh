...
case "${mode}" in
  standalone|clean)
    ...
    env -i \
      ...
      KFPARTICLE_GPU_TEST_VERBOSE_TRACE="${KFPARTICLE_GPU_TEST_VERBOSE_TRACE:-0}" \
      ...
      KFPARTICLE_GPU_BATCH_BENCHMARK_WARMUP_ITERATIONS="${KFPARTICLE_GPU_BATCH_BENCHMARK_WARMUP_ITERATIONS:-}" \
      "${test_executable}" "$@"
    ;;
