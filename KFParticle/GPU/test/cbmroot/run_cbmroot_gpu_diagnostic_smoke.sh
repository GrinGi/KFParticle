...
if [[ -r "${build_dir}/CMakeCache.txt" ]]; then
  ...
  if [[ -n "${configured_source}" && "$(cd "${configured_source}" && pwd)" != "${source_dir}" ]]; then
    echo "FAIL cbmroot-kfp-gpu-diagnostic - build/source mismatch: ${build_dir} was configured from ${configured_source}, script belongs to ${source_dir}" >&2
    exit 2
  fi
fi

if [[ -z "${root_bin}" || ! -x "${root_bin}" ]]; then
  echo "FAIL cbmroot-kfp-gpu-diagnostic - ROOT executable was not found" >&2
  exit 2
fi
...
for library in libxpu.so libCbmRecoBase.so libKFParticle.so libAlgoOffline.so libxpu_Hip.so libKFParticle_Hip.so; do
  if [[ ! -f "${library_dir}/${library}" ]]; then
    echo "FAIL cbmroot-kfp-gpu-diagnostic - missing ${library_dir}/${library}" >&2
    exit 2
  fi
done

mkdir -p "${log_dir}"
log_file="${log_dir}/cbmroot-kfp-gpu-diagnostic-${device}.log"
macro="${script_dir}/cbmroot_gpu_diagnostic_smoke.C"
include_path="${source_dir}:${source_dir}/reco/base:${source_dir}/algo/kfp/gpu"
include_path+=":${source_dir}/external/KFParticle/KFParticle/GPU"
include_path+=":${source_dir}/external/KFParticle/KFParticle:${source_dir}/external/xpu/src"
if [[ -n "${ROOT_INCLUDE_PATH:-}" ]]; then
  include_path+=":${ROOT_INCLUDE_PATH}"
fi

echo "KFParticle CBMRoot GPU diagnostic smoke test"
...
cat "${log_file}"
if ! grep -q '^PASS cbmroot-kfp-gpu-diagnostic' "${log_file}"; then
  echo "FAIL cbmroot-kfp-gpu-diagnostic - macro did not report success" >&2
  exit 1
fi
