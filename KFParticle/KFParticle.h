...
/** @class KFParticle
 ** @brief The main scalar class of KF Particle package, describes particle
 *objects.
 ...
 ** to transport particles, get parameters together with their errors, get
 *distance
...
protected:
   ...
   *from momentum, without any constraints on mass \n
   ** 2 - Energy considered as an independent variable, fitted independently
   ...
   *case the size of the vector is always equal to one; \n
   ** 2) if particle is constructed from other particles - indices of these
   *particles in the same array.
   **/
   ...
   ** Each component (Bx, By, Bz) is approximated with the parabola depending on
   *Z coordinate. Is defined in case of #ifdef NonhomogeneousField.
   **/
...
inline float KFParticle::GetDStoPoint(const float xyz[], float *dsdr) const {
   ...
   ** Also calculates partial derivatives dsdr of the parameter dS over the
   *state vector of the current particle.
   ...
   ** \param[out] dsdr[6] = ds/dr partial derivatives of the parameter dS over
   *the state vector of the current particle
   ** \param[in] param - optional parameter, is used in case if the parameters
   *of the particle are rotated
   ** to other coordinate system (see GetDStoPointBy() function), otherwise fP
   *are used
   **/
#ifdef HomogeneousField
...
#ifdef HomogeneousField
// inline void KFParticle::GetFieldValue( const float * /*xyz*/, float B[] )
// const
//{
/** Calculates the Bx, By, Bz components at the point xyz using approximation of
 *the
 ** magnetic field along the particle trajectory.
 ** \param[in] xyz[3] - X, Y, Z coordiantes of the point where the magnetic
 *field should be calculated
 ** \param[out] B[3] - value of X, Y, Z components of the calculated magnetic
...
inline void KFParticle::GetDStoParticle(const KFParticle &p, float dS[2],
   ...
   ** 2) p - momentum of the particle \n
   ** dS[0] is the transport parameter for the current particle, dS[1] - for the
   *particle "p".
   ** Also calculates partial derivatives dsdr of the parameters dS[0] and dS[1]
   *over the state vectors of the particles:\n
   ...
   ** param2 are parameters of the second particle p.fP. If "HomogeneousField"
   *is defined GetDStoParticleBz() is called,
   ** if "NonhomogeneousField" is defined - GetDStoParticleCBM()
   ** \param[in] p - second particle
   ** \param[out] dS[2] - transport parameters dS for the current particle
   *(dS[0]) and the second particle "p" (dS[1])
   ** \param[out] dsdr[4][6] - partial derivatives of the parameters dS[0] and
   *dS[1] over the state vectors of the both particles
   **/
#ifdef HomogeneousField
...
inline void KFParticle::Transport(float dS, const float *dsdr, float P[],
   ...
   *signed distance and p is
   ** the momentum of the current particle. If "HomogeneousField" is defined
   *TransportBz()
   ** is called, if "NonhomogeneousField" - TransportCBM().
   ** The obtained parameters and covariance matrix are stored to the arrays P
   *and
   ** C respectively. P and C can be set to the parameters fP and covariance
   *matrix fC of the current particle. In this
   ** case the particle parameters will be modified. Dependence of the transport
   *parameter dS on the state vector of the
   ** current particle is taken into account in the covariance matrix using
   *partial derivatives dsdr = d(dS)/d(fP). If
   ** a pointer to F is initialised the transport jacobian F = d(fP new)/d(fP
   *old) is stored.
   ** Since dS can depend on the state vector r1 of other particle or vertex,
   *the corelation matrix
   ** F1 = d(fP new)/d(r1) can be optionally calculated if a pointer F1 is
   *provided. Parameters F and F1 should be either both initialised or both set
   *to null pointer.
   ** \param[in] dS - transport parameter which defines the distance to which
   *particle should be transported
   ** \param[in] dsdr[6] = ds/dr - partial derivatives of the parameter dS over
   *the state vector of the current particle
   ** \param[out] P[8] - array, where transported parameters should be stored
   ** \param[out] C[36] - array, where transported covariance matrix (8x8)
   *should be stored in the lower triangular form
   ** \param[in] dsdr1[6] = ds/dr - partial derivatives of the parameter dS over
   *the state vector of another particle
   ** or vertex
   ** \param[out] F[36] - optional parameter, transport jacobian, 6x6 matrix F =
   *d(fP new)/d(fP old)
   ** \param[out] F1[36] - optional parameter, corelation 6x6 matrix betweeen
   *the current particle and particle or vertex
   ** with the state vector r1, to which the current particle is being
   *transported, F1 = d(fP new)/d(r1)
   **/
#ifdef HomogeneousField
